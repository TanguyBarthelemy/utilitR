...
| `select(matches("...")`     | vérifie une expression régulière (_cf._ fiche [Manipuler des données textuelles](Fiche_donnees_textuelles.qmd)  |
...
| Filtrer sur une variable caractère (voir la [fiche données textuelles](Fiche_donnees_textuelles.qmd)) | `filter(str_detect(TYPEQU, "^A"))`      |
...
| Extraire une sous-chaine de caractères (voir la [fiche données textuelles](Fiche_donnees_textuelles.qmd))  | `mutate(CATEGORIE_EQ = str_sub(TYPEQU, 1L, 1L))`                    |
